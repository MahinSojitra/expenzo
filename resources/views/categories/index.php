...
<div class="col-md-3 d-flex gap-2"><button class="btn btn-primary" type="submit">Filter</button><a class="btn btn-outline-secondary" href="<?=e(url('categories'))?>">Clear</a></div>
</form>
<?php endif; ?>
...
<tbody>
<?php if (!$rows): ?><tr><td colspan="<?=$showOwner ? 7 : 6?>" class="text-center py-5"><p class="text-muted">No categories found.</p><?php if ($actionUrl): ?><a class="btn btn-outline-primary" href="<?=e(url($actionUrl))?>"><?=e($actionLabel)?></a><?php endif; ?></td></tr><?php endif; ?>
...
<td><span class="badge bg-<?=$r['owner_id'] === null ? 'primary' : 'secondary'?>"><?=$r['owner_id'] === null ? 'Global' : 'Personal'?></span></td>
