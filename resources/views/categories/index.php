...
$module = 'categories';
$heading = 'Categories';
$subtitle = can('categories.view_all') ? 'Manage global and user-owned categories.' : 'Use shared categories or create categories available only to you.';
...
<div class="col-md-3"><label for="scope" class="form-label">Scope</label><select id="scope" name="scope" class="form-select"><option value="">All Categories</option><option value="global" <?=selected($filters['scope'],'global')?>>Global Categories</option><option value="personal" <?=selected($filters['scope'],'personal')?>>User-specific Categories</option></select></div>
...
<td><span class="status-badge status-badge--<?=$r['owner_id'] === null ? 'primary' : 'neutral'?>"><i data-feather="<?=$r['owner_id'] === null ? 'globe' : 'user'?>" aria-hidden="true"></i><?=$r['owner_id'] === null ? 'Everyone' : ((int)$r['owner_id'] === (int)auth_user()['id'] ? 'Only me' : 'Owner only')?></span></td>
