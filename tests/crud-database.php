...
$foreignBudget = (int)$pdo->lastInsertId();
$pdo->prepare('UPDATE users SET password_hash=? WHERE id=?')->execute([password_hash('CrudTest123!', PASSWORD_DEFAULT), $other]);
$pdo->exec("INSERT INTO categories(name,owner_id) VALUES('Foreign private category',$other)");
$foreignCategory = (int)$pdo->lastInsertId();
$permissions = $pdo->query('SELECT name,id FROM permissions')->fetchAll(PDO::FETCH_KEY_PAIR);
echo json_encode(['foreignAccount' => $foreignAccount, 'foreignBudget' => $foreignBudget, 'foreignCategory' => $foreignCategory, 'otherUser' => $other, 'permissions' => $permissions]);