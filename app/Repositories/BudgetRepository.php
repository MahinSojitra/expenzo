...
final class BudgetRepository {
 ...
 public function allVisible(array $actor):array{if(\App\Services\Authorization::allows($actor,'finance.view_all')){return Database::connection()->query('SELECT b.*,u.name user_name,c.name category_name,(SELECT COALESCE(SUM(e.amount),0) FROM expenses e WHERE e.user_id=b.user_id AND e.expense_date BETWEEN b.start_date AND b.end_date AND e.status="posted" AND (b.category_id IS NULL OR e.category_id=b.category_id)) spent FROM budgets b JOIN users u ON u.id=b.user_id LEFT JOIN categories c ON c.id=b.category_id ORDER BY b.start_date DESC')->fetchAll();}return $this->allForUser((int)$actor['id']);}
